...
public class Evaluator {
        ...
        if (Operand.check(expressionToken)) {
          ...
          if (!Operator.check(expressionToken)) {
            if (!expressionToken.contains("(")) {
              ...
              String parentheses = expression.substring(firstIndex + 1, secondIndex+1);

              for (int i = firstIndex; i <= secondIndex; i++){
                if (parentheses.contains("(")) {
                  evaluateExpression(expression.substring(firstIndex + 1, secondIndex+1));
                } else {
                  evaluateExpression(expression.substring(firstIndex + 1, secondIndex));
                }
              }
            }
          }
  ...
  public void calculate(Operator operator) {
    while (operatorStack.size() > 0) {
      ...
      Operand operandTwo = operandStack.pop();
      Operand operandOne = operandStack.pop();
      operandStack.push(currentOperator.execute(operandTwo, operandOne));
    }
  }
}